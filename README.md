...
- **Renderer:** **WebGPU (WGSL)** — migrated from WebGL2 in Aug 2026. Raymarch uber-shader ported to WGSL with bind-group layouts (frameData 32 vec4 + lightData 40 vec4 + ModifiersBlock 34 vec4), material texture arrays (12 `texture_2d_array<f32>` + 2 modifier maps), samplers separation, MRT GBuffer (scene + octaNormal+depth+puddleMask), SSR composite, palette quant (256 + LUT 1024x32). Fallback Canvas2D for headless CI (no WebGL2). See `WEBGPU_MIGRATION_PLAN.md` for full architecture.
...
| [webgpu-migration](./tasks/webgpu-migration/) | **COMPLETED Aug 2026 — pure WebGPU.** Migrated entire render pipeline from WebGL2/GLSL 300 es to WebGPU/WGSL. New `gpu-utils.js` (device init, texture array uploads, uniform buffers FrameUniforms 560B + Lighting 800B + Modifiers 544B), WGSL shader-lib port (common, material, pom, raymarch, pbr, chamfer, grid-chamfer, modifiers, scene, ssr), `renderer-gpu.js` pure WebGPU with 5 pipelines (raymarch MRT, SSR, composite, quantize, UI), sprite PBR billboard via separate VS/FS modules (fixed duplicate struct bug), Canvas2D fallback for headless CI (not WebGL2). Fixed WGSL validation errors: `lights.intensity` → private `u_lightIntensity`, `pomOffsetArray` uniform flow → `textureLoad`, `cannot take address of vector component` → local floats, `FrameUniforms` missing in quantize, 19-texture limit. Restored `lighting.json` (ambient 0.36, sun 1.5) and `fog.json` (color) mistuned by live-edit tests. Tests: 20/20 game.spec, 5/5 webgpu-migration.spec, 18/18 game-lighting+chamfer, 11/11 live-edit, 128 unit – all green. | 2026-08-05 |
