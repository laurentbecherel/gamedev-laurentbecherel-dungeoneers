...
- [ ] Editor file system explorer shows `generator.json` under assets config folder — this uses the generic JSON asset editor UI provided by Task 1 (file tree sidebar with Visual Editor tab and Raw JSON tab). Task 2 does NOT add custom editor tabs; dedicated 14-tab UI is Task 7 scope. Reference screenshot in README shows generic editor with generator.json open — author-provided illustration, not solver-required artifact. E2E verifies file appears in tree and is editable/persistable via asset API
