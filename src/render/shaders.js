...
// Part 2: bit-exact alternative to the light uniform arrays above — same values
// packed into an RGBA32F texture (5 texels/row, one row per light). Toggled by
// u_lightsFromTex (0 = read arrays, 1 = read texture). See getLight().
...
// Part 2: light accessor. In array mode this returns exactly the same values the
// two loops used to read inline (bit-identical). In texture mode it unpacks the
// same floats from u_lightTex. Row i = light i; 5 texels: pos.xyz|intensity,
// color.rgb|radius, dir.xyz|type, coneInner|coneOuter|pulseSpeed|pulseAmt,
// noShadow|flickerSpeed|flickerAmount|phase.
struct Light {
  vec3 pos; vec3 color; float intensity; float radius; int ltype;
  vec3 dir; float coneInner; float coneOuter; float pulseSpeed; float pulseAmt;
  int noShadow; float flickerSpeed; float flickerAmount; float phase;
};
Light getLight(int i) {
  ...
  if (u_lightsFromTex == 1) {
    ...
    vec4 t4 = texelFetch(u_lightTex, ivec2(4, i), 0);
    L.pos = t0.xyz; L.intensity = t0.w;
    L.color = t1.xyz; L.radius = t1.w;
    L.dir = t2.xyz; L.ltype = int(t2.w + 0.5);
    L.coneInner = t3.x; L.coneOuter = t3.y; L.pulseSpeed = t3.z; L.pulseAmt = t3.w;
    L.noShadow = int(t4.x + 0.5); L.flickerSpeed = t4.y; L.flickerAmount = t4.z; L.phase = t4.w;
  } else {
    ...
    L.noShadow = u_lightNoShadow[i]; L.flickerSpeed = u_lightFlickerSpeed[i];
    L.flickerAmount = u_lightFlickerAmount[i]; L.phase = u_lightPhase[i];
  }
  return L;
}
...
vec3 pbrShade(vec3 albedo, vec3 N, float rough, float metal, float ao, vec3 emissive, vec3 worldPos, vec3 viewDir) {
  ...
  if (u_pbrEnabled == 0) {
    ...
    for (int i=0;i<12;i++){
      ...
      int lt = L.ltype;
      if (lt==1){
        vec3 sDir = normalize(L.dir);
        float cT = dot(-Lp, sDir);
        float spot = smoothstep(L.coneOuter, L.coneInner, cT);
        atten*=spot;
        if (spot<=0.01) continue;
      }
      // cheap flicker/pulse still applied as attenuation modulation (intensity already flickered on CPU)
      if (lt==2){
        float fs = L.flickerSpeed>0.1?L.flickerSpeed:6.0;
        float fa = L.flickerAmount>0.001?L.flickerAmount:0.12;
        float ph = L.phase;
        float flickAdd = 0.92 + 0.08 * sin(u_time * fs + ph*1.7 + float(i)*0.9) + 0.05 * sin(u_time*fs*1.9+ph*2.3);
        atten *= clamp(flickAdd, 0.68, 1.22);
      } else if (lt==3){
        float ps = L.pulseSpeed; float pa = L.pulseAmt;
        if (ps>0.1 && pa>0.01){ atten *= (1.0 + pa * sin(u_time * ps + L.phase + float(i)*0.7)); }
      }
      float NdotLp = max(dot(N, Lp), 0.0);
      pointContrib += albedo * L.color * L.intensity * atten * NdotLp * shadow * aoPointEff;
    }
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (lType == 1) {
      ...
      float inner = L.coneInner;
      float outer = L.coneOuter;
    ...
    if (lType == 2) {
      float fSpeed = L.flickerSpeed > 0.1 ? L.flickerSpeed : 6.0;
      float fAmt = L.flickerAmount > 0.001 ? L.flickerAmount : 0.12;
      float ph = L.phase;
      ...
      float ps = L.pulseSpeed; float pa = L.pulseAmt;
      if (ps > 0.1 && pa > 0.01) {
        float pulse = 1.0 + pa * sin(u_time * ps + L.phase + float(i)*0.7);
        atten *= pulse;
      }
    }
    ...
    Lo += (kD * albedo / 3.14159265 + specular) * L.color * L.intensity * atten * NdotL * shadow * aoPointEff;
  }
...
// --- Extracted: grid tile chamfer (Task 8, 1m grout). Shared by floor & ceil,
//     wall-hit & no-hit paths. upSign +1 floor / -1 ceil; roughK 0.5 floor / 0.3 ceil.
//     Byte-faithful to the four inline copies it replaces. ---
void applyGridChamfer(inout vec3 albedo, inout vec3 N, inout vec4 rma, inout float ao,
                      vec2 world, float upSign, float gSize, float gDarken, float gBlend,
                      float gRough, float gTrim, float roughK,
                      float gridCreviceSmooth, float gridTStart, float gridTMid, float gridTEnd) {
  vec2 f = fract(world);
  ...
  if (edgeDist < gSize) {
    ...
    vec3 chamN = normalize(vec3(edgeN * 0.6, upSign));
    ...
    rma.r = mix(rma.r * (1.0 - gRough * roughK), rma.r, t);
    if (distX < gSize && distY < gSize) ao *= 0.97;
  }
}

// --- Extracted floor surface shading — unifies the wall-hit and no-hit floor
...
Material floorMaterial(vec2 floorWorld, vec2 ray, float surfaceZ, bool useRoundCham) {
  ...
  if (u_gridDebug == 1) { rma = vec4(0.9,0,0,1); ao = 1.0; emissive = vec3(0); }
    ...
    if (u_chamferEnabled == 1) {
      ...
      if (wd < fS && length(wN) > 0.1) {
        ...
        if (useRoundCham) {
          ...
          targetN = (u_chamferRoundCorners==1) ? roundCham : cham;
        } else targetN = cham;
    ...
    if (u_chamferEnabled == 1 && u_chamferGridEnabled == 1) {
      ...
      applyGridChamfer(albedo, N, rma, ao, floorWorld, 1.0, gSize, gDarken, gBlend, gRough, gTrim, 0.5,
  ...
  if (u_gridDebug == 1) { rma = vec4(0.9,0,0,1); ao = 1.0; emissive = vec3(0); }
    ...
    if (u_chamferEnabled == 1) {
      ...
      if (wd < cS && length(wN) > 0.1) {
        ...
        vec3 cham = normalize(wN + vec3(0.0,0.0,-1.0));
        vec3 targetN;
        if (useRoundCham) targetN = (u_chamferRoundCorners==1) ? normalize(mix(vec3(0.0,0.0,-1.0), cham, t)) : cham;
        else targetN = cham;
        N = normalize(mix(N, targetN, bevel * clamp(u_chamferBlendFloor,0.0,1.0)));
        ao *= mix(u_chamferDarken, 1.0, smoothstep(0.0, creviceSmooth, t));
        float trimBand = smoothstep(tStart, tMid, t) * (1.0 - smoothstep(tMid, tEnd, t));
        albedo += vec3(trimBand * trimCeilAlt);
        rma.r = mix(rma.r * (1.0 - u_chamferRough*coveRoughK), rma.r, t);
      }
    }
    if (u_chamferEnabled == 1 && u_chamferGridEnabled == 1) {
      ...
      applyGridChamfer(albedo, N, rma, ao, ceilWorld, -1.0, gSize, gDarken, gBlend, gRough, gTrim, 0.3,
                       gridCreviceSmooth, gridTStart, gridTMid, gridTEnd);
    }
  }
  Material m;
  ...
  if (hit == 1) {
    ...
    if (wallV_raw < 0.0 || wallV_raw > 1.0) {
      ...
      if (vNorm > horizon) {
        ...
        finalColor = shadeFloorSurface(floorWorld, ray, floorH_atRay, eyeZ, true);
        perpDist = dist;
      } else {
        ...
        finalColor = shadeCeilSurface(ceilWorld, ray, ceilH_atRay, eyeZ, true, 0.35, 0.25);
        perpDist = dist;
      }
    } else {
    ...
    if (vNorm2 > horizon) {
      ...
      finalColor = shadeFloorSurface(floorWorld, ray, floorH, eyeZ2, false);
      perpDist = dist;
    } else {
      float ceilH = 1.15;
      float dist = 0.001;
      ...
      finalColor = shadeCeilSurface(ceilWorld, ray, ceilH, eyeZ2, false, 0.3, 0.2);
      perpDist = dist;
    }
  }
...
// ============================================================================
// Deferred split (toggle with key K). Geometry pass writes material to a 4x
// RGBA32F G-buffer; lighting pass reads it and runs pbrShade + fog/tonemap.
// Both reuse GLSL_WORLD_PRELUDE so the material/lighting GLSL is shared with the
// forward shader. Not bit-exact vs forward: wall side-darken is folded into
// albedo and viewDir uses a single eye height (small, measured deltas).
// ============================================================================
export const fsGeometrySrc = GLSL_WORLD_PRELUDE + `
layout(location = 0) out vec4 gA; // albedo.rgb, perpDist (a)
layout(location = 1) out vec4 gN; // worldNormal.rgb, ao (a)
...
void main() {
  ...
  float cameraX = 2.0 * fragCoord.x / u_resolution.x - 1.0;
  vec2 rayDir = vec2(cos(u_playerAngle), sin(u_playerAngle));
  vec2 plane = vec2(-rayDir.y, rayDir.x) * tan(u_fov * 0.5);
  vec2 ray = rayDir + plane * cameraX;
  ...
  if (hit == 1) {
    ...
    if (wallV_raw < 0.0 || wallV_raw > 1.0) {
      ...
      float wallV = clamp(wallV_raw, 0.0, 1.0);
      float matId = max(1.0, cellType);
      vec2 uvAtlas = atlasUV(matId, vec2(wallU, wallV), u_atlasWalls, u_texSize);
      vec3 NgeomFlat = vec3(0.0); vec3 tangentFlat = vec3(0.0); vec3 bitangent = vec3(0.0,0.0,1.0);
      if (side == 0) { NgeomFlat = vec3(float(-stepDir.x),0.0,0.0); tangentFlat = vec3(0.0, ray.x > 0.0 ? -1.0 : 1.0, 0.0); }
      else { NgeomFlat = vec3(0.0, float(-stepDir.y), 0.0); tangentFlat = vec3(ray.y < 0.0 ? -1.0 : 1.0, 0.0, 0.0); }
      vec3 Ngeom = NgeomFlat; vec3 tangent = tangentFlat;
      if (hasCornerRound) {
        vec3 cornerGeom = cornerNormal;
        if (u_cornerMode == 0) { vec3 n2 = (side == 0) ? vec3(0.0,(wallU<0.5?-1.0:1.0),0.0) : vec3((wallU<0.5?-1.0:1.0),0.0,0.0); cornerGeom = normalize(NgeomFlat + n2); }
        Ngeom = normalize(mix(NgeomFlat, cornerGeom, clamp(nMix, 0.0, 1.0)));
        float dotTN = dot(tangentFlat, Ngeom);
        vec3 tOrtho = tangentFlat - dotTN * Ngeom;
        if (dot(tOrtho, tOrtho) < 0.000001) { tOrtho = vec3(-Ngeom.y, Ngeom.x, 0.0); if (dot(tOrtho, tangentFlat) < 0.0) tOrtho = -tOrtho; }
        tangent = normalize(tOrtho);
      }
      vec3 worldPos = vec3(hitPos.x, hitPos.y, u_playerHeight + (wallV - 0.5));
      vec3 viewDir = normalize(vec3(u_playerPos, u_playerHeight) - worldPos);
      vec3 viewTS = vec3(dot(viewDir, tangent), dot(viewDir, bitangent), dot(viewDir, Ngeom));
      vec2 uvPOM = uvAtlas;
      if (u_pomEnabled == 1) { uvPOM = uvAtlas + pomOffset(u_wallHeight, uvAtlas, viewTS, u_pomWall, u_pomSteps); }
      vec3 albedoRaw = texture(u_wallAlbedo, uvPOM).rgb;
      vec3 normalTSw = decodeNormal(texture(u_wallNormal, uvPOM).rgb);
      vec4 rmaW = texture(u_wallRoughMetal, uvPOM);
      vec3 emissiveW = albedoRaw * emissiveAlbedoMul * rmaW.b * emissiveStrength;
      vec3 Nw = normalize(tangent * normalTSw.x + bitangent * normalTSw.y + Ngeom * normalTSw.z);
      if (hasCornerRound) { albedoRaw += vec3(albBoost); rmaW.r *= roughMul; rmaW.a *= aoMul; }
      if (u_chamferEnabled == 1) {
        {
          float fS = max(u_chamferFloorSize, 0.04);
          float cS = max(u_chamferCeilSize, 0.04);
          if (wallV < fS) {
            float t = wallV / fS; float bevel = 1.0 - smoothstep(0.0,1.0,t);
            vec3 up = vec3(0.0,0.0,1.0); vec3 chamGeom = normalize(Ngeom + up);
            vec3 chamRound = normalize(mix(up, chamGeom, smoothstep(0.0,1.0,t)));
            vec3 targetN = (u_chamferRoundCorners==1) ? chamRound : chamGeom;
            Nw = normalize(mix(Nw, targetN, bevel * clamp(u_chamferBlendFloor,0.0,1.0)));
            rmaW.a *= mix(u_chamferDarken, 1.0, smoothstep(0.0, creviceEnd, t));
            float trim = smoothstep(tStart, 0.32, t) * (1.0 - smoothstep(0.32, 1.0, t));
            albedoRaw += vec3(trim * trimFloor); rmaW.r *= mix(0.58, 1.0, t);
          }
          if ((1.0 - wallV) < cS) {
            float t = (1.0 - wallV) / cS; float bevel = 1.0 - smoothstep(0.0,1.0,t);
            vec3 down = vec3(0.0,0.0,-1.0); vec3 chamGeom = normalize(Ngeom + down);
            vec3 targetN = (u_chamferRoundCorners==1) ? normalize(mix(down, chamGeom, smoothstep(0.0,1.0,t))) : chamGeom;
            Nw = normalize(mix(Nw, targetN, bevel * clamp(u_chamferBlendFloor,0.0,1.0)));
            rmaW.a *= mix(u_chamferDarken, 1.0, smoothstep(0.0, creviceEnd, t));
            float trim = smoothstep(tStart, 0.32, t) * (1.0 - smoothstep(0.32, 1.0, t));
            albedoRaw += vec3(trim * trimCeil); rmaW.r *= mix(0.62, 1.0, t);
          }
        }
        if (!hasCornerRound) {
          float vS = max(u_chamferWallSize, 0.04); float e = min(wallU, 1.0 - wallU);
          if (e < vS) {
            float t = e / vS; float bevel = 1.0 - smoothstep(0.0,1.0,t);
            vec3 n2;
    ...
    ivec2 pc = ivec2(floor(u_playerPos));
    float pfH = 0.0;
    if (pc.x >= 0 && pc.y >= 0 && pc.x < int(u_mapSize.x) && pc.y < int(u_mapSize.y)) { vec4 pmd = texelFetch(u_mapTex, pc, 0); pfH = clamp(pmd.g - 0.5, -0.6, 0.6); }
    float eyeZ2 = 0.5 + pfH * eyeFactor;
    if (vNorm2 > horizon) {
...
export const fsLightingSrc = GLSL_WORLD_PRELUDE + `
uniform sampler2D u_gA;
uniform sampler2D u_gN;
uniform sampler2D u_gP;
uniform sampler2D u_gM;
out vec4 outColor;

void main() {
  vec4 a = texture(u_gA, v_uv);
  vec4 n = texture(u_gN, v_uv);
  vec4 p = texture(u_gP, v_uv);
  vec4 mm = texture(u_gM, v_uv);
