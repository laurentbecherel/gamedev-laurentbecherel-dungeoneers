// map-ui.js — consistent circle+triangle icons.
// Design system strict final:
// - All icons (entrance, exit, player) SAME radius max(6,cs*0.9), SAME border 1px #2a2a2a, SAME triangle proportions, NO white.
// - Entrance & Exit literally same except triangle orientation:
//   Both gold #c9a84c circle + dark #2a2a2a triangle, entrance up (-PI/2), exit down (PI/2)
//   Player: muted green #58805c circle + dark #2a2a2a triangle facing angle (same style, only color differs for role + direction)
// - Player green [88,128,92] fits parchment, trail same green [88,128,92] configurable via JSON.
...
function toCssColor(c, fallback) { if (!c) return fallback; if (typeof c === "string") return c; if (Array.isArray(c)) { if (c.length >= 4) return `rgba(${c[0]|0},${c[1]|0},${c[2]|0},${c[3]/255})`; if (c.length >= 3) return `rgb(${c[0]|0},${c[1]|0},${c[2]|0})`; } return fallback; }
...
function fillRect(buf, w, h, x0, y0, rw, rh, r, g, b, a = 255) { const x1 = Math.min(w, x0+rw), y1 = Math.min(h, y0+rh); for (let y=Math.max(0,y0); y<y1; y++) for (let x=Math.max(0,x0); x<x1; x++) setPixel(buf,w,x,y,r,g,b,a); }
...
function resolveLayout(uiCfg) {
  ...
  return {
    ...
    legend: { swatch: layout.legend?.swatch ?? 12, gap: layout.legend?.gap ?? 8, itemWidth: layout.legend?.itemWidth ?? 90, panelAlpha: layout.legend?.panelAlpha ?? 220, borderAlpha: layout.legend?.borderAlpha ?? 220 },
...
function resolveDiscoveryCfg(discovery, discoveryCfg) {
  const fromInstance = discovery?._cfg || {}; const fromFile = discoveryCfg || {};
  const reveal = fromFile.reveal || fromInstance.reveal || {}; const trail = fromFile.trail || fromInstance.trail || {};
  const dither = reveal.dither || fromInstance.reveal?.dither || {};
  const pDirFile = fromFile.playerDir || reveal.playerDir || {}; const pDirInst = fromInstance.playerDir || fromInstance.reveal?.playerDir || {};
  const playerDir = { ...pDirInst, ...pDirFile };
  return {
    reveal: { enabled: reveal.enabled ?? true, peekDistance: reveal.peekDistance ?? 1, animationDuration: reveal.animationDuration ?? 400, dither: { enabled: dither.enabled ?? true, pattern: dither.pattern ?? "random" }, undiscovered: { hide: true }, playerDir: { enabled: playerDir.enabled ?? true, size: playerDir.size ?? 0, color: playerDir.color ?? [42,42,42], opacity: playerDir.opacity ?? 0.95 } },
    trail: { enabled: trail.enabled ?? true, color: trail.color ?? [88,128,92], opacity: trail.opacity ?? 0.45, lineWidth: trail.lineWidth ?? 2.0, dash: trail.dash ?? [5,4], cap: "butt", join: "miter", onlyDiscovered: trail.onlyDiscovered ?? true },
    playerDir: { enabled: playerDir.enabled ?? true, size: playerDir.size ?? 0, color: playerDir.color ?? [42,42,42], opacity: playerDir.opacity ?? 0.95 },
  };
}
...
function shouldDrawCell(x, y, discovery, newlySet, animProgress, cfg) {
  ...
  if (pattern === "bayer") { const b = bayerMatrix4(); return b[y & 3][x & 3] / 16 < animProgress; }
  else { const order = discovery.getDiscoveryOrder ? discovery.getDiscoveryOrder(x, y) : 0; return hash01(x, y, order) < animProgress; }
}
function trailColorToCss(color) { if (!color) return "#58805c"; if (typeof color === "string") return color; if (Array.isArray(color)) return `rgb(${color[0]|0},${color[1]|0},${color[2]|0})`; return "#58805c"; }
...
function getUnifiedIconRadius(cs) { return Math.max(6, Math.floor(cs * 0.9)); }
...
export function generateMapTextureData(dungeon, mode = "role", player = null, uiCfg = {}, discovery = null, animProgress = 1, discoveryCfg = null) {
      ...
      if (dungeon) {
        ...
        const getCellColor = (x, y, gv, room) => {
          if (gv === 0) {
            ...
            else if (mode === "zone") { const zi = room ? ["Entry","Antechamber","Depths","Sanctum","Exit"].indexOf(room.zone) : 2; const t = ZONE_TINTS[Math.max(0, Math.min(4, zi))] || ZONE_TINTS[2]; return `rgb(${t[0]},${t[1]},${t[2]})`; }
            else { const fm = d.floorMat ? d.floorMat[y*d.w+x] : 1; const shades = [PALETTE.materials.floor1, PALETTE.materials.floor2]; return shades[(fm-1)%2]; }
          } else {
            if (mode === "material") { const shades = [PALETTE.materials.wall1, PALETTE.materials.wall2]; return shades[(gv-1)%2]; }
        ...
        if (discoveryEnabled) {
          for (let y=0;y<d.h;y++) for (let x=0;x<d.w;x++) {
            ...
            if (gv===0) { ctx.fillStyle = toCssColor(getCellColor(x,y,0,room), "#6a6a6a"); ctx.fillRect(ox+x*cs, oy+y*cs, cs, cs); }
            else { if (!hasFloorNeighbor(x,y)) continue; ctx.fillStyle = toCssColor(getCellColor(x,y,gv,room), "#2a2a2a"); ctx.fillRect(ox+x*cs, oy+y*cs, cs, cs); }
          }
        } else {
          const wallT = Math.max(1, cs * 0.18); const roomCornerR = Math.max(2, Math.min(8, cs * 0.5));
          d.rooms.forEach(r => {
            const rx = ox + r.x*cs, ry = oy + r.y*cs; const rw = r.w*cs, rh = r.h*cs;
            let color; if (mode === "role") color = ROLE_COLORS[r.role] || PALETTE.materials.floor1; else if (mode === "zone") { const zi = ["Entry","Antechamber","Depths","Sanctum","Exit"].indexOf(r.zone); const t = ZONE_TINTS[Math.max(0, Math.min(4, zi))] || ZONE_TINTS[2]; color = `rgb(${t[0]},${t[1]},${t[2]})`; } else { color = PALETTE.materials.floor1; }
            ctx.fillStyle = toCssColor(PALETTE.wallDark, "#2a2a2a"); roundRectPath(ctx, rx - wallT/2, ry - wallT/2, rw + wallT, rh + wallT, roomCornerR + wallT/2); ctx.fill();
            ctx.fillStyle = toCssColor(color, "#9a9a9a"); roundRectPath(ctx, rx, ry, rw, rh, roomCornerR); ctx.fill();
          });
          for(let y=0;y<d.h;y++) for(let x=0;x<d.w;x++) {
            const i=y*d.w+x, gv=d.grid[i], room=roomByCell.get(i); if (room) continue;
            if (gv===0) { ctx.fillStyle = toCssColor(getCellColor(x,y,0,null), "#6a6a6a"); ctx.fillRect(ox+x*cs, oy+y*cs, cs, cs); }
            else { if (!hasFloorNeighbor(x,y)) continue; ctx.fillStyle = toCssColor(getCellColor(x,y,gv,null), "#2a2a2a"); ctx.fillRect(ox+x*cs, oy+y*cs, cs, cs); }
          }
        }
        // Entrance & Exit literally SAME except triangle orientation
        ...
        const playerTriCss = toCssColor(L.player.dirColor || [42,42,42], "#2a2a2a");

        const drawStairIdentical = (r, isExit) => {
          ...
          // SAME for both: gold circle + dark triangle, only angle differs
          const angle = isExit ? Math.PI/2 : -Math.PI/2;
          drawCircleWithTriangle(ctx, cx, cy, unifiedRadius, goldCss, border, wallDarkCss, border, angle, 0.85);
        };
        d.rooms.filter(r=>r.role==="entrance").forEach(r=>drawStairIdentical(r,false));
        d.rooms.filter(r=>r.role==="exit").forEach(r=>drawStairIdentical(r,true));
        ...
        if (cfg.trail.enabled && discovery) {
          ...
          if (path.length > 1) {
            ...
            ctx.save();
            ctx.globalAlpha = cfg.trail.opacity ?? 0.45;
            ctx.strokeStyle = trailColor;
            ctx.lineWidth = cfg.trail.lineWidth ?? 2.0;
            ...
            for (let i=0;i<path.length-1;i++) {
              ...
              if (cfg.trail.onlyDiscovered) { if (!discovery.isDiscovered(a.x,a.y) || !discovery.isDiscovered(b.x,b.y)) { started = false; continue; } }
        ...
        const items = mode==="role" ? Object.entries(PALETTE.roles).filter(([k])=>k!=="corridor").map(([k,c])=>[ROLE_DISPLAY[k]||k, toCssColor(c, "#888")]) : mode==="zone" ? PALETTE.zones.map(z => [z.name, toCssColor(z.hex || `rgb(${z.rgb[0]},${z.rgb[1]},${z.rgb[2]})`, "#a0875f")]) : [["Wall 1", toCssColor(PALETTE.materials.wall1, "#4a4a4a")],["Wall 2", toCssColor(PALETTE.materials.wall2, "#5a5a5a")],["Floor 1", toCssColor(PALETTE.materials.floor1, "#a0a0a0")],["Floor 2", toCssColor(PALETTE.materials.floor2, "#8a8a8a")]];
        const swatch = L.legend.swatch, gap = L.legend.gap, textPad = 6, itemW = L.legend.itemWidth;
        ...
        for(const [label,col] of items) {
          ctx.fillStyle = col; roundRectPath(ctx, lx, ly-6, swatch, swatch, 2); ctx.fill();
          ctx.strokeStyle = toCssColor(PALETTE.parchmentPanelBorder, "#8b7355"); ctx.lineWidth = 0.5; roundRectPath(ctx, lx, ly-6, swatch, swatch, 2); ctx.stroke();
          ctx.fillStyle = toCssColor(PALETTE.textLight, "#3a3020"); ctx.textAlign = "left"; ctx.fillText(label, lx + swatch + textPad, ly); lx += itemW;
        }
      }
  ...
  const scanEvery = L.parchment.scanlineEvery;
  for (let y=0;y<h;y++){ const useScan=(scanEvery>0 && y % scanEvery === 0); const col=useScan?scan:bg; for(let x=0;x<w;x++) setPixel(buf,w,x,y,col[0],col[1],col[2],alpha); }
  ...
  const roomByCell = new Map();
  d.rooms.forEach(r=>{ for(let dy=0;dy<r.h;dy++)for(let dx=0;dx<r.w;dx++) roomByCell.set((r.y+dy)*d.w+(r.x+dx), r); });
  const getCellColor = (x,y,gv,r)=>{ if(gv===0){ const col = r?(PALETTE.roles[r.role]||PALETTE.materials.floor1):PALETTE.roles.corridor; return hexToRgbArray(col); } return hexToRgbArray(PALETTE.wallDark); };
  const isFloor = (x,y)=> x>=0&&y>=0&&x<d.w&&y<d.h&&d.grid[y*d.w+x]===0;
  const hasFloorNeighbor = (x,y)=> isFloor(x-1,y)||isFloor(x+1,y)||isFloor(x,y-1)||isFloor(x,y+1);
  for(let y=0;y<d.h;y++) for(let x=0;x<d.w;x++){ if (!shouldDrawCell(x,y,discovery,newlySet,animProgress,cfg)) continue; const i=y*d.w+x, gv=d.grid[i], r=roomByCell.get(i); if (r) { const c=getCellColor(x,y,0,r); fillRect(buf,w,h,ox+x*cs,oy+y*cs,cs,cs,c[0],c[1],c[2],255); } else { if(gv===0){ const c=getCellColor(x,y,0,null); fillRect(buf,w,h,ox+x*cs,oy+y*cs,cs,cs,c[0],c[1],c[2],255); } else if(hasFloorNeighbor(x,y)){ const c=getCellColor(x,y,gv,null); fillRect(buf,w,h,ox+x*cs,oy+y*cs,cs,cs,c[0],c[1],c[2],255); } } }
  ...
  d.rooms.filter(r=>r.role==="entrance"||r.role==="exit").forEach(r=>{
    if(!r.stairWall) return;
    if (discoveryEnabled) { const mx = (r.stairWall.x1 + r.stairWall.x2)/2, my = (r.stairWall.y1 + r.stairWall.y2)/2; if (!shouldDrawCell(Math.floor(mx), Math.floor(my), discovery, newlySet, animProgress, cfg)) return; }
    const sw=r.stairWall; const mx=(sw.x1+sw.x2)/2, my=(sw.y1+sw.y2)/2; const cx=Math.floor(ox+mx*cs), cy=Math.floor(oy+my*cs);
    fillCircle(buf,w,h,cx,cy,uniR, gold[0],gold[1],gold[2],255);
    strokeCircle(buf,w,h,cx,cy,uniR, wallDark[0],wallDark[1],wallDark[2],255,1);
  });
  if(player){
    ...
    strokeCircle(buf,w,h,px,py,uniR,42,42,42,255,1);
  }
  if (cfg.trail.enabled && discovery) {
    const path = discovery.getPath ? discovery.getPath() : [];
    ...
    for (let i=0;i<path.length-1;i++) {
      ...
      for (let s=0;s<=steps;s++) { if (cfg.trail.dash && (s % (cfg.trail.dash[0]+cfg.trail.dash[1])) >= cfg.trail.dash[0]) continue; const x = Math.floor(ax + (bx-ax)*s/steps), y = Math.floor(ay + (by-ay)*s/steps); if (x>=0&&y>=0&&x<w&&y<h) setPixel(buf,w,x,y,trailRgb[0],trailRgb[1],trailRgb[2],trailAlpha); }
    }
  }
  ...
  items.forEach(([label,col])=>{ fillRect(buf,w,h,lx,ly-7,swatch,swatch,col[0],col[1],col[2],255); lx+=itemW; });
  return buf;
}
