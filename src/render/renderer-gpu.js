...
function packFrameUniforms(buf, cfg) {
  ...
  wF32(FRAME_OFFSETS.ssrDepthRange, cfg.ssrDepthRange ?? 20);
...
export class GPURenderer {
  ...
  render(dungeon, player, timeSec) {
    ...
    const frameUniformValues = {
      ...
      ssrDepthRange: this._resolveConfigValue(cfg, ['ssr.depthRange','rendering.ssrDepthRange'], 20),
      ...
      chamferFloorSize: chamferCfg.size?.floor ?? chamferCfg.floorSize ?? 0.30,
      chamferCeilSize: chamferCfg.size?.ceil ?? chamferCfg.ceilSize ?? 0.24,
      chamferWallSize: chamferCfg.size?.wall ?? chamferCfg.wallSize ?? 0.28,
      chamferCornerRadius: chamferCfg.cornerRadius ?? 0.15,
      chamferDarken: chamferCfg.shading?.darken ?? chamferCfg.darken ?? 0.55,
      chamferRoundCorners: chamferCfg.roundCorners?1:1,
      chamferBlendFloor: chamferCfg.blend?.floor ?? 0.85,
      chamferBlendWall: chamferCfg.blend?.wall ?? 0.85,
      chamferRough: chamferCfg.rough ?? 0.3,
      chamferFloor: 0.12, chamferCeil:0.10, chamferWall:0.08,
      chamferTrimFloor: chamferCfg.trim?.floor ?? 0.22,
      chamferTrimCeil: chamferCfg.trim?.ceil ?? 0.18,
      chamferTrimWall: chamferCfg.trim?.wall ?? 0.16,
      chamferTrimFloorAlt: 0.18, chamferTrimCeilAlt:0.14,
      chamferCreviceEnd: 0.12, chamferCreviceSmoothEnd:0.30,
      chamferTrimStart:0.08, chamferTrimMid:0.35, chamferTrimEnd:1.0,
      chamferGridEnabled: 1,
      chamferGridFloorSize: 0.05, chamferGridCeilSize:0.05,
      chamferGridFloorDarken:0.88, chamferGridCeilDarken:0.90,
      chamferGridFloorTrim:0.06, chamferGridCeilTrim:0.04,
      chamferGridFloorRough:0.35, chamferGridCeilRough:0.30,
      chamferGridFloorBlend:0.85, chamferGridCeilBlend:0.80,
      chamferGridCreviceEnd:0.12, chamferGridCreviceSmoothEnd:0.30,
      chamferGridTrimStart:0.10, chamferGridTrimMid:0.35, chamferGridTrimEnd:1.0,
      cornerEnabled: this.cornerEnabled,
      cornerRadius: cornersCfg.radius ?? 0.15,
      cornerMode: cornersCfg.mode ?? 2,
      cornerInner: cornersCfg.inner ? 1 : 1,
      cornerBandNear: cornersCfg.search?.bandNear ?? 0.08,
      cornerBandFarExtra: cornersCfg.search?.bandFarExtra ?? 0.12,
      cornerBandFarFactor: cornersCfg.search?.bandFarFactor ?? 2.0,
      cornerSectorThresh: cornersCfg.search?.sectorThresh ?? 0.5,
      cornerNormalMix: cornersCfg.shading?.normalMix ?? 0.92,
      cornerAlbedoBoost: cornersCfg.shading?.albedoBoost ?? 0.05,
      cornerRoughMul: cornersCfg.shading?.roughMul ?? 0.82,
      cornerAoMul: cornersCfg.shading?.aoMul ?? 0.96,
      shadowBiasN: shadowsCfg.bias?.traceNormalOffset ?? 0.10,
      shadowBiasDir: shadowsCfg.bias?.dirOffset ?? 0.06,
      shadowSunFactor: shadowsCfg.sun?.shadowFactor ?? 0.25,
      shadowPointFactor: shadowsCfg.point?.shadowFactor ?? 0.15,
      shadowSunMax: shadowsCfg.dda?.maxSteps ? shadowsCfg.sun?.maxDist ?? 20 : 20,
      shadowPointEps: shadowsCfg.point?.distEpsilon ?? 0.1,
      shadowNormalThresh: shadowsCfg.bias?.normalThresh ?? 0.02,
