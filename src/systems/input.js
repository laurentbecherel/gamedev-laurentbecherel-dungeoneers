 ...
 *
 * Best practice mapping table (code vs label):
 *   Code | QWERTY | AZERTY | Action
 *   KeyW | W      | Z      | Forward (ZQSD Z)
 *   KeyZ | Z      | W      | Forward courtesy (W label on AZERTY)
 *   KeyS | S      | S      | Back
 *   KeyA | A      | Q      | StrafeLeft (Q label AZERTY)
 *   KeyD | D      | D      | StrafeRight
 *   KeyQ | Q      | A      | TurnLeft (A label AZERTY)
 *   KeyE | E      | E      | TurnRight
 *   Digit1-8 | 1-8 | &é"''(-è_ç (+Shift for digit) | Debug toggles, works without Shift because we check code
 *
 * All gameplay shortcuts check codes, never `key`. `key` only for text input.
 */

const CODE_MAP = {
  forward: [''KeyW'', ''KeyZ'', ''ArrowUp''], // KeyW = W QWERTY = Z AZERTY, KeyZ = Z QWERTY = W AZERTY courtesy
  back: [''KeyS'', ''ArrowDown''],
  strafeLeft: [''KeyA''], // KeyA = A QWERTY = Q AZERTY → Q in ZQSD
  strafeRight: [''KeyD''],
  turnLeft: [''KeyQ'', ''ArrowLeft''], // KeyQ = Q QWERTY = A AZERTY → A turn in AZERTY
  turnRight: [''KeyE'', ''ArrowRight''],
  // Alt mappings for user who expects QWERTY Q/A both to strafe left in free mode
  strafeLeftAlt: [''KeyQ''],

  // Action / debug keys via code
  regen: [''KeyR''],
  map: [''KeyM''],
  gridToggle: [''KeyG''],
  bobToggle: [''KeyV'', ''KeyB''],
...
export class Input {
  constructor(canvas) {
    ...
    this.pressed = new Set(); // Set of codes
    ...
    this._buffer = null; // {type, age}
    this._bufferTimeout = 0.3;

    this._onKeyDown = (e) => {
      if (e.code) this.pressed.add(e.code);
    };
    this._onKeyUp = (e) => {
      if (e.code) this.pressed.delete(e.code);
    };
    this._onBlur = () => {
      this.pressed.clear();
    };
  ...
  // Update called each frame — returns input intent for free mode
  // and drives player directly in grid mode (prototypes pattern)
  update(dt, player, map) {
    // Allow signature update() with no args for legacy Game loop — returns {forward, strafe, turn, mouseDX}
    if (!player || !map) {
      // Legacy fallback: used by Game if it only expects intent object
    ...
    // Override hold timings from player config if present
    ...
    if (gridMode) {
      ...
      const tryImmediate = (type, fn, holdKey) => {
        if (fn()) {
          acted = true;
          this._hold[holdKey] = 0;
          this._buffer = null;
          return true;
        }
        if (player.moveLerp < 1 || player.turnLerp < 1) {
          this._buffer = { type, age: 0 };
        }
        return false;
      };
      ...
      if (idle && !acted) {
        const tryHold = (down, timerKey, fn) => {
          ...
          if (fn()) {
            this._hold[timerKey] = this._holdInitial - this._holdRepeat;
            this._buffer = null;
            return true;
          } else {
            this._hold[timerKey] = this._holdInitial - this._holdRepeat;
    ...
    this.prevPressed = new Set(this.pressed);
    // Return intent for Game that still expects it (free mode mostly)
