...
export class Game {
  ...
  async _onKeyDown(e) {
    ...
    if (code === "Digit9" || code === "Numpad9") {
      const mode = this.renderer.cycleModifiersDebug ? this.renderer.cycleModifiersDebug() : (this.renderer.toggleModifiers ? this.renderer.toggleModifiers() : 0);
      const name = this.renderer.getModifiersModeName ? this.renderer.getModifiersModeName() : (mode ? "ON" : "OFF");
      this._showHud("Modifiers ["+mode+"]: " + name + " - 9 cycles OFF/ON/MOSS/DAMAGED/WATER/PUDDLE/BLOOD/DUST/COMBINED");
      return;
    }
  }
