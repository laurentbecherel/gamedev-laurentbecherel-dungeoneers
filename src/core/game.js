...
export class Game {
  ...
  async _onKeyDown(e) {
    ...
    if (code === "Digit6" || code === "Numpad6") { const v = this.renderer.cyclePBRDebug(); const names=["Normal - OFF","Moss Noise mask (3D FBM)","Moss Nearwall/Env mask (corners+floor seams+room biome)","Moss Material mask (AO/Height/Rough combo)","Moss Combined mask (final)","Puddle mask","Moss R + Puddle B"]; this._showHud("PBR Debug: " + (names[v]||"Mode "+v) + " (" + v + ") - moss decomposed: noise|env|material|combined"); return; }
