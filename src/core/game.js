 ...
 export class Game {
   constructor(canvas) {
     ...
     this.lastTime = 0;
     this.showMap = false;
     this._bobPresetIdx = 1; // default preset index
   ...
   async init() {
     ...
     // Task 4: Input takes canvas for pointer-lock, handles AZERTY-safe code mapping internally
   ...
   _loop(time) {
     ...
     if (this.renderer && this.renderer.isReady() && this.player && this.input && this.dungeon) {
       // Task 4: Input now owns dt + player + map and does hold repeat + buffer + mouse look
   ...
   async _onKeyDown(e) {
     // Use code for layout-agnostic shortcuts — AZERTY fix
     const code = e.code || '''';
     const key = (e.key || '''').toLowerCase();

     // Regen and map (R/M) — code based
     ...
     // Debug toggles 1-8 — use code Digit1-8 for AZERTY compatibility (press physical 1 position = & on AZERTY still works)
